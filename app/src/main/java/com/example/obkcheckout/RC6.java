...
public class RC6 {
    ...
    private static byte[] paddingKey(byte[] key){
        ...
        int paddedLen = key.length + (4 - l);
        byte[] padded = new byte[paddedLen]; // JVM zero-fills
    ...
    public static byte[] encrypt(byte[] data, byte[] key) {
        ...
        int lenght = 16 - ((data.length % 16) > 0 ? (data.length % 16) : 16);
    ...
    private static byte[] deletePadding(byte[] input){
        ...
        if (((input.length - count) % 16) > 0)
            count++;
        else { }

        byte[] tmp = new byte[input.length - count];
